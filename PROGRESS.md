...
### Phase 4 — 進階功能 (已完成 ✅)
- [x] 進階搜尋 (全文檢索、標籤過濾) — 伺服器端全文檢索、標籤多選篩選、debounce 搜尋
- [x] 時間軸 + 地圖連動 (點時間軸事件 ➔ 地圖定位) — 時間軸卡片加入地圖按鈕，地圖頁支援 flyTo 聚焦指定事件
- [x] 統計儀表板 (事件類別分布、年分布) — `/dashboard` 頁面，類別/年代條形圖
- [x] 主題遊覽路線 (預設歷史路徑) — `/tours` 頁面，5 條主題路線 (建黨到建國、改革開放、人權、外交、經濟)
